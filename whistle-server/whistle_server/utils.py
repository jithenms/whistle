...
class EncryptedField(models.TextField):
    ...
    def from_db_value(self, value, expression, connection):
        if value:
            cipher_hash = base64.b64encode(
                hashlib.sha1(value.encode()).digest()
            ).decode()
            cache = self._redis_client.get(f"whistle_server:{cipher_hash}")
            if cache:
                return cache.decode()
            else:
                ...
                self._redis_client.set(
                    f"whistle_server:{cipher_hash}", data, ex=self.cache_expiry
                )
                return data
        return value

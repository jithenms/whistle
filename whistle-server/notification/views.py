...
from authentication.middleware import ClientAuthentication, ServerAuthentication, IsValidExternalId
...
class NotificationViewSet(CreateAPIView, mixins.ListModelMixin, mixins.UpdateModelMixin, mixins.RetrieveModelMixin,
    ...
    def get_authenticators(self):
        if self.request.method == 'POST' or self.request.headers.get('X-External-Id') is None:
            return [ServerAuthentication()]
        return super(NotificationViewSet, self).get_authenticators()

    def get_queryset(self):
        if self.request.headers.get('X-External-Id') is not None:
            user = User.objects.get(external_id=self.request.headers.get('X-External-Id'))
    ...
    def get_permissions(self):
        if self.action != 'create' and self.request.headers.get('X-External-Id') is not None:
